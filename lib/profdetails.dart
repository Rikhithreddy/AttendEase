...
class Profdetails extends StatefulWidget {
  final signupid;
  final uid;
  const Profdetails({super.key, required this.signupid, required this.uid});
...
class _ProfdetailsState extends State<Profdetails> {
  ...
  Future<void> insertProfdata() async {
    await supabase.from('profdetails').insert({
      'prof_id': widget.signupid,
      'prof_name': _fullnamecontoller.text.trim(),
